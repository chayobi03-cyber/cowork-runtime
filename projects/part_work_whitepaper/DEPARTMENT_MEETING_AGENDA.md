...
1. 소파트별로 실제 반복 업무가 뭔지 파트장들이 3개씩만 먼저 말해줄 수 있는가?
2. 사내에서 LLM/인트라 검색을 어디까지 쓸 수 있는가 (완전 금지 / 사내 LLM만 / 외부 AI 일부 허용)?
3. 공유폴더 외에 이미 쓰고 있는 문서 도구(SharePoint 등)가 있는가?
4. Topic 판정(Approve/Reject/Needs changes)을 창엽님 혼자 할 것인가, 파트장 협의를 거칠 것인가?
5. 파트원들에게 "1분 카드 작성"을 실제로 요청할 채널/타이밍이 있는가(정기 회의, 슬랙 등)?
