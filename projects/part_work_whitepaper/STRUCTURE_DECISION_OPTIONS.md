...
- 음성/화면녹화 입력은 조건부 지원(사람의 사전 전처리 필요)이며, 이를 자동화하는 도구는
  **v1 MVP 범위에서 명시적으로 제외** → `TOPIC_REGISTRY.yaml`의 TOPIC-0002(hold)로 등록,
  v1 완료 및 회의 이후 별도 프로젝트로 분리 착수 여부 판단 (`WRITING_METHODOLOGY.md` C-03/C-05
  "PC 환경에서 실제 지원 여부" 참조)
...
- [ ] Topic 판정 주체 (창엽님 단독 / 파트장 협의) — **회의 전 결정 불가** (파트장 의견 필요)
- [ ] 운영 위치 1순위 후보 (사내 시스템 확인 후) — **회의 전 결정 불가** (사내 확인 선행 필요)
- [ ] 외부 AI 사용 가능 범위 (법무/보안 확인 후) — **회의 전 결정 불가** (법무/보안 확인 선행)

3개 모두 회의 전 단독 결정이 어려우며, 회의는 "결정"보다 "확인 담당자·일정 배정"이 우선
목적이 된다 (DEPARTMENT_MEETING_AGENDA.md 회의 후 액션 참조).
