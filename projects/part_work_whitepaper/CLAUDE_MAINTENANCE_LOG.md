...
## 2026-07-13 — LOW 4건 반영

- LOW-1: `representative_output`을 실제 파일 경로 + 현재 진행 상태로 갱신 (미래형 표현 제거)
- LOW-2: `next_action` 축약, 상세 근거는 `CLAUDE_MAINTENANCE_LOG.md` 참조로 이전 (roster
  가독성 확보)
- LOW-3: `TOPIC_CARD_TEMPLATE.md`의 "왜 이렇게 판단하는가" → **"판단 근거"**로 재명명하고,
  Level A의 "왜 필요한가"와 다른 질문임을 괄호로 명시 — 필드명 유사성으로 인한 혼동 방지
- LOW-4: **정책 결정** — 과거 커밋 메시지(영어)는 rebase/rewrite 하지 않는다 (2.1
  비가역적 작업 원칙상 이력 재작성은 별도 승인 필요 사안이고, 이 정도 스타일 문제로는 ROI가
  낮음). **앞으로 새 커밋 메시지는 한국어로 작성**한다.
